...
from ...data_sources.odps.odps_reading_helper import ODPSReader
from ...data_sources.odps.odps_writing_helper import ODPSWritingHelper
...
class ODPSDataSource(DataSource):
    def __init__(self, rank: int, num_worker: int,
                 column_conf: config_fields.ColumnFields, source_conf):
        ...
        for rows in batch_iterator:
            for row in rows:
                yield rcd_builder.build(
                    ...
                    label=row[label_offset] if label_offset > 0 else None,
                    group=row[group_offset] if group_offset > 0 else None,
                    weight=row[weight_offset] if weight_offset > 0 else None,
                    append_info=row[append_offset:] if append_offset > 0 else None)
